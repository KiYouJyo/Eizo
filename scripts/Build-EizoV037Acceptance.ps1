...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.3.7-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v037'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v037'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v037'
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.3.7.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v037-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.3.7-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.3.7.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.3.7' -PackageVersion '0.3.7.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.3.7-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.3.7-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.3.7 acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
