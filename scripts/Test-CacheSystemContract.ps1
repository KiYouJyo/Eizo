...
Assert-Contains $cacheView 'CardBackgroundFillColorDefaultBrush' 'Video-cache rows do not have a card background.'
Assert-Contains $cacheView 'Property="CornerRadius"' 'Video-cache item highlight does not share the card corner radius.'
Assert-Contains $cacheView '<ControlTemplate TargetType="ListViewItem">' 'Video-cache cards still rely on the default ListViewItem surface.'
Assert-Contains $cacheView 'UseSystemFocusVisuals' 'Default outer focus visuals can still draw around cache cards.'
Assert-Contains $cacheView 'Property="BorderThickness"' 'Cache-card template does not explicitly control the outer border.'
