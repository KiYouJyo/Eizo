...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.3.9-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v039'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v039'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v039'
    ...
    Write-Host '== Validate v0.3.9 metadata transport resilience =='
    & ./scripts/Test-MetadataScanLifecycleContract.ps1
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.3.9.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v039-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.3.9-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.3.9.0') {
    ...
    Write-Host '== Stage published Metadata v0.2.12 externally and simulate restart =='
    $metadataRoot = Join-Path $componentsRoot 'Recognition'
    $metadataVersionRoot = Join-Path $metadataRoot 'versions\0.2.12'
    ...
    $metadataZip = Join-Path $runnerTemp 'Eizo.Recognition.Runtime-v0.2.12-x64.zip'
    Invoke-WebRequest -Uri 'https://github.com/KiYouJyo/Eizo.Metadata/releases/download/v0.2.12/Eizo.Recognition.Runtime-v0.2.12-x64.zip' -OutFile $metadataZip -UseBasicParsing
    $metadataExpected = '7b2754ec0f822c505b898d6d74b94b7b88439751638727de9441681167bfd7a1'
    $metadataActual = (Get-FileHash -LiteralPath $metadataZip -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($metadataActual -ne $metadataExpected) { throw "Published Metadata v0.2.12 digest mismatch: $metadataActual" }
    ...
    foreach ($name in $metadataAssemblies) {
        if (-not (Test-Path (Join-Path $metadataVersionRoot "bin\$name"))) {
            throw "Metadata v0.2.12 module is missing: $name"
        }
    }
    '{"Version":"0.2.12"}' | Set-Content -LiteralPath (Join-Path $metadataRoot 'pending.json') -Encoding utf8NoBOM
    ...
    if ([string]$active.Version -ne '0.2.12') { throw "Unexpected active Metadata version: $($active.Version)" }
    ...
    if ($runtimeLine -notmatch '\tversion=0\.2\.12\texternal=True\tprobe=' -or
        $runtimeLine -notmatch ([regex]::Escape($externalAssembly))) {
        throw "Metadata v0.2.12 state was promoted but the external Recognition runtime was not actually invoked. Log:`n$runtimeLine"
    }
    ...
    if ($metadataRuntimeLine -notmatch '\tversion=0\.2\.12\texternal=True\tprobe=ok\t' -or
        $metadataRuntimeLine -notmatch ([regex]::Escape($externalCore)) -or
        $metadataRuntimeLine -notmatch ([regex]::Escape($externalProviders))) {
        throw "Metadata v0.2.12 state was promoted but external Core/Providers were not actually invoked. Log:`n$metadataRuntimeLine"
    }

    $running | Stop-Process -Force -ErrorAction SilentlyContinue
    Write-Host 'Metadata v0.2.12 external restart activation and real Recognition/Core/Providers calls PASS.'

    Write-Host '== Build one-click acceptance assets =='
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.3.9' -PackageVersion '0.3.9.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.3.9-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.3.9-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.3.9 provider-resilience acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
