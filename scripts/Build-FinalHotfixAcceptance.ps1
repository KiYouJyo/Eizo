...
$artifactDir = Join-Path $env:RUNNER_TEMP 'Eizo-v0.2.1-final-hotfix-acceptance'
Remove-Item $artifactDir -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $artifactDir | Out-Null

$signedBundle = Join-Path $artifactDir 'Eizo_0.2.1.0_x64_final-hotfix.msixbundle'
...
if (-not $pkg -or [string]$pkg.Version -ne '0.2.1.0') {
    throw 'Eizo v0.2.1 acceptance package was not registered correctly.'
}
...
Write-Host "Final v0.2.1 hotfix acceptance package ready: $signedBundle"
