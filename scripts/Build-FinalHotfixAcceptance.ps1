...
if ($packageVersion -ne '0.2.2.1') {
    throw "Unexpected package version: $packageVersion"
}

$bundleOutput = Join-Path $env:RUNNER_TEMP 'Eizo-CompactPaneTheme-AppPackages'
...
$artifactDir = Join-Path $env:RUNNER_TEMP 'Eizo-v0.2.2.1-compact-pane-theme-acceptance'
Remove-Item $artifactDir -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $artifactDir | Out-Null

$signedBundle = Join-Path $artifactDir 'Eizo_0.2.2.1_x64_final-hotfix.msixbundle'
...
$cer = Join-Path $artifactDir 'Eizo-v0.2.2-AppPublisher.cer'
...
if (-not $pkg -or [string]$pkg.Version -ne '0.2.2.1') {
    throw 'Eizo v0.2.2.1 acceptance package was not registered correctly.'
}
...
Write-Host "Compact-pane theme acceptance package ready: $signedBundle"
