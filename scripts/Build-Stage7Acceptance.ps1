...
do {
    ...
    $windowReady = @($running | Where-Object {
        try {
            $_.Responding -and $_.MainWindowHandle -ne [IntPtr]::Zero
        }
        catch {
            $false
        }
...
while ((Get-Date) -lt $deadline -and -not $windowReady)
...
Write-Host "Signed Stage 7 visible-window smoke PASS. Window process(es): $((@($running | Where-Object { $_.MainWindowHandle -ne [IntPtr]::Zero }).ProcessName) -join ', ')"
...
if ($survivors.Count -eq 0) {
    throw 'Eizo created a window but did not remain alive for the sustained launch smoke.'
}

Write-Host "Signed Stage 7 sustained-window smoke PASS. Process(es): $($survivors.ProcessName -join ', ')"
