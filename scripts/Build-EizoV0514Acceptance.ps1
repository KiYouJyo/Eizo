...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.5.14-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v0514'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v0514'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v0514'
    ...
    Write-Host '== Validate v0.5.14 scraping architecture simplification and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.5.14.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v0514-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.5.14-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.5.14.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.5.14' -PackageVersion '0.5.14.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.5.14-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.5.14-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.5.14 scraping architecture simplification acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
