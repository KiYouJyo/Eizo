...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.6.0-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v060'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v060'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v060'
    ...
    Write-Host '== Validate v0.6.0 player and detail experience and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.6.0.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v060-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.6.0-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.6.0.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.6.0' -PackageVersion '0.6.0.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.6.0-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.6.0-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.6.0 player and detail experience acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
