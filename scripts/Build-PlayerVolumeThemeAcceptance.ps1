...
$signedDir = Join-Path $env:RUNNER_TEMP 'Eizo-updater-controls-signed'
...
$signedMsix = Join-Path $signedDir 'Eizo_0.2.1.0_x64_updater-controls.msix'
...
$cer = Join-Path $signedDir 'Eizo-v0.2.1-AppPublisher.cer'
...
Write-Host 'Running the same updater bundle-signature verifier against the signed acceptance package...'
& dotnet run --project tools/Eizo.UpdateVerifierProbe/Eizo.UpdateVerifierProbe.csproj --configuration Release -- $signedMsix
if ($LASTEXITCODE -ne 0) {
    throw "Updater signature verifier probe failed: $LASTEXITCODE"
}
Write-Host 'Updater signature verifier probe PASS.'
...
Write-Host "Signed updater/controls acceptance package ready: $signedMsix"
