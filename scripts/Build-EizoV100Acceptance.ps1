...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v1.0.0-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v100'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v100'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v100'
    ...
    Write-Host '== Validate v1.0.0 first-run guide and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_1.0.0.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v100-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v1.0.0-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '1.0.0.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '1.0.0' -PackageVersion '1.0.0.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v1.0.0-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v1.0.0-x64-one-click.zip'
    ...
    Write-Host "Eizo 1.0.0 first-run guide acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
