...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v1.2.1-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v121'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v121'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v121'
    ...
    Write-Host '== Validate v1.2.1 manual single-file video cache and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_1.2.1.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v121-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v1.2.1-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '1.2.1.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '1.2.1' -PackageVersion '1.2.1.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v1.2.1-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v1.2.1-x64-one-click.zip'
    ...
    Write-Host "Eizo 1.2.1 manual single-file video cache acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
