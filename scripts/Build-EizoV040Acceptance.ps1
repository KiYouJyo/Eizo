...
try {
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.4.0.5_x64.msixbundle'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.4.0.5') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.4.0' -PackageVersion '0.4.0.5'
