...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.3.11-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v0311'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v0311'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v0311'
    ...
    Write-Host '== Validate v0.3.11 library UI geometry, season-family aggregation and Metadata cache contract =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.3.11.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v0311-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.3.11-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.3.11.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.3.11' -PackageVersion '0.3.11.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.3.11-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.3.11-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.3.11 UI/season-family/cache acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
