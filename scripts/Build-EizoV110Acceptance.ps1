...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v1.1.0-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v110'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v110'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v110'
    ...
    Write-Host '== Validate v1.1.0 picture-in-picture and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_1.1.0.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v110-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v1.1.0-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '1.1.0.0') {
    ...
    Write-Host "Eizo 1.1.0 picture-in-picture acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
