...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.5.3-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v053'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v053'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v053'
    ...
    Write-Host '== Validate v0.5.3 TMDB details Stage 2 Slice C and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.5.3.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v053-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.5.3-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.5.3.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.5.3' -PackageVersion '0.5.3.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.5.3-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.5.3-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.5.3 TMDB details Stage 2 Slice C acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
