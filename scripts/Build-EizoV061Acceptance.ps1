...
try {
    ...
    $assets = Join-Path $runnerTemp 'Eizo-v0.6.1-acceptance'
    $appPackages = Join-Path $runnerTemp 'Eizo-AppPackages-v061'
    $bundleExtract = Join-Path $runnerTemp 'Eizo-bundle-extract-v061'
    $oneClickStaging = Join-Path $runnerTemp 'Eizo-one-click-v061'
    ...
    Write-Host '== Validate v0.6.1 visual assets and prior release regressions =='
    ...
    $bundlePath = Join-Path $assets 'Eizo_0.6.1.0_x64.msixbundle'
    ...
    $pfx = Join-Path $runnerTemp 'eizo-v061-release-signing.pfx'
    ...
    $cer = Join-Path $assets 'Eizo-v0.6.1-AppPublisher.cer'
    ...
    if (-not $pkg -or [string]$pkg.Version -ne '0.6.1.0') {
    ...
    Get-AppxPackage -Name Eizo -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $componentsRoot -Recurse -Force -ErrorAction SilentlyContinue
    & ./packaging/New-GitHubOneClickInstallerPackage.ps1 -SignedBundlePath $bundlePath -PublicCertificatePath $cer -OutputDirectory $oneClickStaging -DisplayVersion '0.6.1' -PackageVersion '0.6.1.0'
    $packageRoot = Join-Path $oneClickStaging 'Eizo-v0.6.1-x64-one-click'
    & ./packaging/Test-GitHubOneClickInstallerPackage.ps1 -ReleaseDirectory $packageRoot
    $oneClickZip = Join-Path $assets 'Eizo-v0.6.1-x64-one-click.zip'
    ...
    Write-Host "Eizo 0.6.1 visual assets acceptance PASS. Assets=$assets"
}
finally {
    Pop-Location
}
