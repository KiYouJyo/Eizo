...
Bangumi 条目页升级为可直接浏览社区内容的桌面入口，本版不改动本地媒体库刮削链。根据上游反馈，Eizo 不申请社区内容发布授权，因此 0.4.5 保持社区内容只读；已登录用户仍可使用无需 Turnstile 的 reaction。
...
- 动画日志正文现按 Bangumi 图片路径规则加载日志内图片，不再把 `cf/xx/...jpg` 当普通文本显示；日志评论区继续使用真实评论 API 读取评论。
...
本版不提供短评、长评、讨论或回复的发布入口，也不再包含 Turnstile 发布链。播放进度写回仍不在本版范围内。
